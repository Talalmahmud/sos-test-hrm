...
import AddButton from "@/components/control/AddButton";
...
const AddBook = (props: Props) => {
  ...
  const getAllBooks = async () => {
    try {
      setIsLoading(true);
      const res = await axios.get(EndPoint.ALL_BOOK, {
        params: {
        ...
        <table className="w-full  text-[#121212] ">
          <thead className=" bg-[#DBDDE2] sticky top-0 z-10">
            <tr className=" text-left">
              <th className=" px-[4px] py-4 text-[16px] font-semibold ">
                Name
              </th>
              <th className="  py-4 text-[16px] font-semibold ">Category</th>
              <th className="  py-4 text-[16px] font-semibold ">Department</th>
              <th className="  py-4 text-[16px] font-semibold ">Semester</th>
              <th className="  py-4 text-[16px] font-semibold ">Price</th>
              <th className="  py-4 text-[16px] font-semibold ">Quantity</th>
              <th className="  py-4 text-[16px] font-semibold ">Remaining</th>
              <th className="  py-4 text-[16px] font-semibold ">Sale</th>
              <th className="  py-4 text-[16px] font-semibold ">Action</th>
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              ...
              bookList?.map((book: any, index: number) => (
                <tr
                  key={index}
                  className={` border-b-[1px] text-left border-gray_light hover:bg-bg_card text-[14px] cursor-pointer `}
                >
                  <td className=" px-[4px] font-semibold  text-purple py-[4px]">
                    {book?.book_name}
                  </td>
                  <td className=" py-[10px]">{book?.category}</td>
                  <td className=" py-[10px]">{book?.department?.full_name}</td>
                  <td className=" py-[10px]">{book?.semester}</td>
                  <td className=" py-[10px]">{book?.price}</td>
                  <td className=" py-[10px]">{book?.quantity}</td>
                  <td className=" py-[10px]">
                    {book?.quantity - book?.total_sells}
                  </td>
                  <td className=" py-[10px]">{book?.total_sells}</td>
                  <td>
                    {
                      <div className=" flex items-center justify-center gap-[10px]">
                        <div
                          className=" flex items-center justify-center  cursor-pointer "
                          onClick={() => editAction(book?.id)}
                        >
                          <svg
                        ...
                        {/* <div
                        className=" flex items-center justify-center  cursor-pointer "
                        onClick={() => deleteCatagory(book?.id)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="19"
