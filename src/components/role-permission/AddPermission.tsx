...
const AddPermission = (props: Props) => {
  ...
  const getAll = async () => {
    if (user?.access_token) {
      setIsLoading(true);
      const res = await axios.get(EndPoint.GET_ALL_PERMISSION, {
  ...
  const deleteCatagory = async () => {
    try {
      const res = await axios.delete(EndPoint.DELETE_PERMISSION + groupId, {
        headers: { Authorization: "Bearer " + user?.access_token },
      });
            ...
            {isLoading ? (
              ...
              listData?.map((item: any, index: any) => (
                ...
                >
                  ...
                  <td className=" py-4 border-[1px] border-gray_light flex flex-wrap items-center justify-center gap-2">
                    {item?.Permission?.map((item: any, index: any) => (
                      <p
                        className=" h-[19px] flex justify-center items-center  px-[4px] bg-title text-white text-[14px] rounded-[16px] "
                        key={index}
                      >
                        {item?.name}
                      </p>
