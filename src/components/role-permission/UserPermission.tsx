...
import RolePermissionSkeleton from "../skeleton/RolePermissionSkeleton";

type Props = {};

const UserPermission = (props: Props) => {
  const { user } = useContext(AuthContext);
  const [toggle, setToggle] = useState(false);
  const [editToggle, setEditToggle] = useState(false);
  const [listData, setListData] = useState([]);
  const [EmployeePermissionId, setEmployeePermissionId] = useState<any>(null);
  ...
  const getAll = async () => {
    if (user?.access_token) {
      setIsLoading(true);
      const res = await axios.get(EndPoint.GET_ALL_EMPLOYEE_PERMISSION, {
  ...
  const clikOndeleteButton = (item_id: any) => {
    setEmployeePermissionId(item_id);
    setDeleteWarn(true);
  };
  ...
  const deleteCatagory = async () => {
    try {
      const res = await axios.delete(
        EndPoint.DELETE_EMPLOYEE_PERMISSION + EmployeePermissionId,
        {
          headers: { Authorization: "Bearer " + user?.access_token },
        }
      );
      toast.success("Permission assign is deleted.");
      setDeleteWarn(false);
      getAll();
    } catch (error) {
      toast.error("Permission assign is not deleted.");
      setDeleteWarn(false);
      console.log(error);
    }
  };
      ...
      <div className=" border-[1px] border-gray_light ">
        <table className="w-full table-fixed  text-center text-[#121212] ">
          <thead className=" bg-[#DBDDE2] sticky top-0 z-10">
            <tr>
              <th className=" w-[200px] px-4 py-2 text-[16px] font-normal ">
                Role
              </th>
              <th className=" px-4 py-2 text-[16px] font-normal ">Users</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              listData?.map((item: any, index: any) => (
                <tr
                  key={index}
                  className={`  hover:bg-bg_card   ${
                    index % 2 === 0 && "bg-bg_orange"
                  } `}
                >
                  {/* Your code to render each row */}
                  <td
                    className=" border-[1px] border-gray_light"
                    style={{ width: "30px" }}
                  >
                    {item?.group?.name}
                  </td>
                  <td className=" py-4 border-[1px] border-gray_light flex flex-wrap items-center justify-center gap-1">
                    {item?.user?.map((item: any, index: any) => (
                      <p
                        className=" h-[20px] flex justify-center items-center  px-[8px] bg-title text-white text-[14px] rounded-[16px] "
                        key={index}
                      >
                        {item?.name}
                      </p>
                  ...
                  >
                    {
                      <div className=" flex items-center justify-center gap-[10px]">
                        ...
                        <div
                          className=" flex items-center justify-center  cursor-pointer "
                          onClick={() => clikOndeleteButton(item?.id)}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="19"
                    ...
                    {editToggle && (
                      <CustomModal setClose={setEditToggle}>
                        <UserPermissionForm
                          permission_id={EmployeePermissionId}
                          listUpdate={getAll}
