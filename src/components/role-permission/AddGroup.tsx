...
import ControlForm from "../control/catagory/ControlForm";
...
const AddGroup = (props: Props) => {
  ...
  const getAll = async () => {
    if (user?.access_token) {
      setIsLoading(true);
      const res = await axios.get(EndPoint.GET_ALL_GROUP, {
  ...
  const deleteCatagory = async () => {
    try {
      const res = await axios.delete(EndPoint.DELETE_GROUP + groupId, {
        headers: { Authorization: "Bearer " + user?.access_token },
      });
      toast.success("Group is deleted.");
      setDeleteWarn(false);
      getAll();
    } catch (error) {
        ...
        <table className="w-full table-fixed  text-center text-[#121212] ">
          <thead className=" bg-[#DBDDE2] sticky top-0 z-10">
            <tr>
              ...
              <th className=" px-4 py-2 text-[16px] font-normal ">Status</th>
              <th className=" px-4 py-2 text-[16px] font-normal ">Actions</th>
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
