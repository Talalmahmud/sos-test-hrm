...
import IndependentPackageForm from "./IndependentPackageForm";
...
const IndependentPackage = (props: Props) => {
  ...
  const getAll = async () => {
    ...
    const res = await axios.get(EndPoint.GET_ALL_INDEPENDENT_PACKAGE);
  ...
  const deleteCustomDate = async (custom_id: any) => {
    try {
      const res = await axios.delete(
        EndPoint.DELETE_INDEPENDENT_PACKAGE + custom_id
      );
      ...
      <div className=" shadow-md ">
        <table className="w-full text-left text-[#121212] ">
          <thead className=" bg-[#DBDDE2] sticky top-0 z-10">
            <tr>
              <th className=" pl-1text-[16px] font-semibold ">Type</th>
            ...
            {isLoading ? (
              ...
              listData?.map((item: any, index: any) => (
                ...
                >
                  ...
                  <td>{item?.independent_schedule?.title}</td>
