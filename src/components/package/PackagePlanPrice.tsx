...
import PackagePlanPriceForm from "./PackagePlanPriceForm";
import Pagination from "../Pagination";
import DeleteCaution from "../DeleteCaution";

type Props = {};

const PackagePlanPrice = (props: Props) => {
  ...
  const deleteItem = async () => {
    try {
      const res = await axios.delete(
        EndPoint.DELETE_PACKAGE_PLAN_PRICE + deleteId
      );
      toast.success("Package Plan is deleted.");
      setDeleteWarn(false);
      getAll();
    } catch (error: any) {
      if (error?.response) {
  ...
  return (
    <div className=" min-w-full flex flex-col gap-[24px]">
      ...
      <div className=" shadow-md ">
        <table className="w-full   text-[#121212] ">
          <thead className=" bg-[#DBDDE2] ">
            <tr>
              <th className=" text-left py-2 text-[16px] font-semibold ">
                Category
              </th>
              <th className="  text-left py-2 text-[16px] font-semibold ">
                Package
              ...
              listData?.map((item: any, index: any) => (
                ...
                >
                  ...
                  <td className=" py-4 ">
                    ...
                    {item?.Semester_Quantity
                      ? item?.Semester_Quantity
                      : item?.package_layer}
                    )
                  </td>
                  <td className=" py-4 ">{item?.package?.package_name}</td>
                  <td className=" py-4">{item?.plan?.plan_name}</td>
...
export default PackagePlanPrice;
