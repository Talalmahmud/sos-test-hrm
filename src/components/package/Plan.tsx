...
const Plan = (props: Props) => {
  ...
  const getAll = async () => {
    setIsLoading(true);
    const res = await axios.get(EndPoint.GET_ALL_PLAN, {
      params: {
        limit: pageSize,
  ...
  const deletePlan = async (plan_id: any) => {
    try {
      const res = await axios.delete(EndPoint.DELETE_PLAN + plan_id);
  ...
  return (
    <div className=" flex flex-col gap-[24px]">
      ...
      <div className=" shadow-md ">
        <table className="w-full table-fixed rounded-bl-[16px] rounded-br-[16px] text-left text-[#121212] ">
