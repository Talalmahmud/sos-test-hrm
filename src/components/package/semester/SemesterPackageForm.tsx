...
const SemesterPackageForm = ({
    ...
    if (custom_id) {
      try {
        const res = await axios.get(
          EndPoint.GET_SEMESTER_PACKAGE_BY_ID + custom_id
        );
        ...
        setSelectedSchedule(resResult?.semester_schedule);
        setPackageData({
  ...
  const addCustomDate = async () => {
    if (selectedCategory !== null) {
      const bodyData = {
        schedule_type: "Semester Schedule Based",
        category: selectedCategory?.value,
        package_name: packageData?.package_name,

        status: packageData?.status,
        semester_schedule: selectedSchedule?.map((item: any) => item?.value),
      };

      try {
        const res = await axios.post(EndPoint.ADD_SEMESTER_PACKAGE, bodyData);
        setPackageData({});
        setEmptyCheck(false);
        listUpdate();
        toast.success("Semester Package is add.");
        formClose(false);
      } catch (error: any) {
        if (error?.response) {
    ...
    if (selectedCategory !== null) {
      ...
      try {
        const res = await axios.patch(
          EndPoint.UPDATE_SEMESTER_PACKAGE + custom_id,
          bodyData
        );
        setPackageData({});
        setEmptyCheck(false);
        listUpdate();
        toast.success("Semester Package is update.");
        formClose(false);
      } catch (error: any) {
        if (error?.response) {
          toast.error(Object.values(error.response.data).join(""));
  ...
  const getScheduleList = async () => {
    const res = await axios.get(EndPoint.GET_DROPDOWN_SEMESTER_SERVICE);
    const result = await res.data;
  ...
  return (
    <div className=" w-[850px] rounded-[16px] bg-green text-white p-[24px] grid grid-cols-2 justify-center gap-[24px]">
      ...
      <div className=" flex flex-col gap-[4px]">
        ...
        {selectedSchedule.length === 0 && emptyCheck && (
          <p className=" text-[#FFCC00] font-semibold text-[12px]">
...
export default memo(SemesterPackageForm);
