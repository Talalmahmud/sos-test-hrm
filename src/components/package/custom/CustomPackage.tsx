...
const CustomPackage = (props: Props) => {
  ...
  const getAll = async () => {
    setIsLoading(true);
    const res = await axios.get(EndPoint.GET_ALL_CUSTOMDATE_PACKAGE);
  ...
  const deleteCustomDate = async (custom_id: any) => {
    try {
      const res = await axios.delete(
        EndPoint.DELETE_CUSTOMDATE_PACKAGE + custom_id
      );
      toast.success("Package is deleted.");
      getAll();
    } catch (error) {
      toast.error("Package is not deleted.");
      ...
      <div className=" shadow-md ">
        <table className="w-full text-left text-[#121212] ">
          <thead className=" bg-[#DBDDE2] sticky top-0 z-10">
            <tr>
              <th className=" pl-1py-2 text-[16px] font-semibold ">Type</th>
            ...
            {isLoading ? (
              ...
              listData?.map((item: any, index: any) => (
                ...
                >
                  ...
                  <td>{item?.custom_schedule?.title}</td>
