...
const CustomPackageForm = ({
    ...
    if (custom_id) {
      try {
        const res = await axios.get(
          EndPoint.GET_CUSTOMDATE_PACKAGE_BY_ID + custom_id
        );
  ...
  const addCustomDate = async () => {
    if (selectedCategory !== null) {
      const bodyData = {
        schedule_type: "Custom Fixed Date Based",
        category: selectedCategory?.value,
        package_name: packageData?.package_name,

        status: packageData?.status,
        custom_schedule: selectedSchedule?.value,
      };

      try {
        const res = await axios.post(EndPoint.ADD_CUSTOMDATE_PACKAGE, bodyData);
        setPackageData({});
        setEmptyCheck(false);
        listUpdate();
    ...
    if (selectedCategory !== null) {
      ...
      try {
        const res = await axios.patch(
          EndPoint.UPDATE_CUSTOMDATE_PACKAGE + custom_id,
          bodyData
        );
        setPackageData({});
        setEmptyCheck(false);
  ...
  const getScheduleList = async () => {
    const res = await axios.get(EndPoint.GET_CUSTOM_DATE_DROPDOWN);
    const result = await res.data;
...
export default memo(CustomPackageForm);
