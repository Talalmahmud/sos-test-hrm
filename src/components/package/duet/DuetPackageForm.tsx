...
const DuetPackageForm = ({
    ...
    if (custom_id) {
      try {
        const res = await axios.get(
          EndPoint.GET_DUET_PACKAGE_BY_ID + custom_id
        );
        ...
        setSelectedSchedule(resResult?.duet_schedule);
        setPackageData({
  ...
  const addCustomDate = async () => {
    if (selectedCategory !== null) {
      const bodyData = {
        schedule_type: "DUET Exam Based",
        category: selectedCategory?.value,
        package_name: packageData?.package_name,

        status: packageData?.status,
        duet_schedule: selectedSchedule?.map((item: any) => item?.value),
      };

      try {
        const res = await axios.post(EndPoint.ADD_DUET_PACKAGE, bodyData);
        setPackageData({});
        setEmptyCheck(false);
        listUpdate();
        toast.success("Duet Package is add.");
        formClose(false);
      } catch (error: any) {
        if (error?.response) {
    ...
    if (selectedCategory !== null) {
      ...
      try {
        const res = await axios.patch(
          EndPoint.UPDATE_DUET_PACKAGE + custom_id,
          bodyData
        );
        setPackageData({});
        setEmptyCheck(false);
  ...
  const getScheduleList = async () => {
    const res = await axios.get(EndPoint.GET_DROPDOWN_DUET_SERVICE);
    const result = await res.data;
...
export default memo(DuetPackageForm);
