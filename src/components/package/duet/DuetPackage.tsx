...
import DuetPackageForm from "./DuetPackageForm";
...
const DuetPackage = (props: Props) => {
  ...
  const getAll = async () => {
    ...
    const res = await axios.get(EndPoint.GET_ALL_DUET_PACKAGE);
    const resData = await res.data;
    setListData(resData?.results);
    setIsLoading(false);
  };

  const deleteCustomDate = async (custom_id: any) => {
    try {
      const res = await axios.delete(EndPoint.DELETE_DUET_PACKAGE + custom_id);
      ...
      <div className=" shadow-md ">
        <table className="w-full text-left text-[#121212] ">
          <thead className=" bg-[#DBDDE2] ">
            <tr>
              <th className=" pl-1 py-2 text-[16px] font-semibold ">Type</th>
              <th className="py-2 text-[16px] font-semibold ">Category</th>
              <th className="py-2 text-[16px] font-semibold ">Name</th>
              <th className="py-2 text-[16px] font-semibold ">Schedule</th>
            ...
            {isLoading ? (
              ...
              listData?.map((item: any, index: any) => (
                ...
                >
                  ...
                  <td className=" py-4">{item?.schedule_type}</td>
                  <td>{item?.category?.category_name}</td>
                  <td>{item?.package_name}</td>
                  <td className="">
                    <div className=" flex flex-col gap-[2px]">
                      {item?.duet_schedule?.map((item: any, index: number) => (
                        <p
                          key={index}
                          className=" my-[2px] bg-green text-white"
                        >
                          {item?.session?.session_year}
                        </p>
                      ))}
                    </div>
                  </td>
                    ...
                    <div className=" flex items-center">
                      {" "}
                      {
                        <div className=" flex items-center justify-center gap-[10px]">
                          ...
                          <div
                            className=" flex items-center justify-center  cursor-pointer "
                            onClick={() => deleteCustomDate(item?.id)}
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="19"
