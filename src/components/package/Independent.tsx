...
const Independent = (props: Props) => {
  ...
  const [independentId, setIndependentId] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(false);

  const getAll = async () => {
    setIsLoading(true);
    const res = await axios.get(EndPoint.GET_ALL_INDEPENDENT_DATE);
    const resData = await res.data;
    setListData(resData?.results);
    setIsLoading(false);
  };

  const deleteCatagory = async (independent_id: any) => {
    try {
      const res = await axios.delete(
        EndPoint.DELETE_INDEPENDENT_DATE + independent_id
      );
      toast.success("Independent days is deleted.");
      getAll();
    } catch (error: any) {
      if (error?.response) {
      ...
      <div className=" shadow-md">
        <table className="w-full  text-left text-[#121212] ">
          <thead className=" bg-[#DBDDE2]">
            <tr>
              <th className=" pl-1 py-2 text-[16px] font-semibold ">Serial</th>
              <th className=" py-2 text-[16px] font-semibold ">Name</th>
              <th className=" py-2 text-[16px] font-semibold ">Days</th>
            ...
            {isLoading ? (
              ...
              listData?.map((item: any, index: any) => (
                ...
                >
                  ...
                  <td className=" py-4">{item?.days_quantity}</td>
