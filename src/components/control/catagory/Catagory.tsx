...
const Catagory = (props: Props) => {
  ...
  const getAll = async () => {
    setIsLoading(true);
    const res = await axios.get(EndPoint.GET_CATAGORY);
  ...
  const deleteCatagory = async () => {
    try {
      const res = await axios.delete(EndPoint.DELETE_CATAGORY + catagoryId);
      ...
      <div className=" border-[1px] border-gray_light ">
        <table className="w-full table-fixed  text-left text-[#121212] ">
          <thead className=" bg-[#DBDDE2] sticky top-0 z-10">
            <tr>
              <th className="  py-2 text-[16px] font-semibold ">Course Name</th>
