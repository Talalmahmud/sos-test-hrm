...
const Semester = (props: Props) => {
  ...
  const [probidanList, setProbidanList] = useState([]);
  const [searchSession, setSearchSession] = useState<any>("");
  const [isLoading, setIsLoading] = useState(false);

  const getAllProbidan = async () => {
    setIsLoading(true);
    const res = await axios.get(EndPoint.GET_DROPDOWN_SESSION);
    const resData = await res.data;
    setProbidanList(resData?.results);
    setIsLoading(false);
  };
  const getAll = async () => {
    setIsLoading(true);
    const res = await axios.get(
      EndPoint.GET_SEMESTER_SCHEDUL + `?search=${searchSession}`
    );
    const resData = await res.data;
  ...
  const editAction = (sId: any) => {
    setSemesterId(sId);
    setEditToggle(!editToggle);
  };

  const deleteSemester = async (semester_id: any) => {
    try {
      const res = await axios.delete(
        EndPoint.DELETE_SEMESTER_SCHEDUL + semester_id
      );
      getAll();
      toast.success("Semester session is deleted.");
    } catch (error: any) {
      if (error?.response) {
    ...
    <div>
      <div className=" flex flex-col gap-[24px]">
        <div className=" grid grid-cols-2 gap-[24px]">
          <AddButton
            title="Add semester Expire date"
            boxWidth={372}
          ...
          <div className=" flex-grow flex items-center">
            <input
              ...
              className=" border-[1px] outline-none rounded-bl-[8px] rounded-tl-[8px] w-[743px] px-[24px] h-[40px] "
        ...
        <div className=" border-[1px] border-gray_light">
          <table className="w-full text-left text-[#121212] ">
            <thead className=" bg-[#DBDDE2] sticky top-0 z-10">
              <tr>
                <th className=" pl-1 py-2 text-[16px] font-semibold ">
                  Sesssion
                </th>
                <th className="  py-2 text-[16px] font-semibold ">Start</th>
                <th className="  py-2 text-[16px] font-semibold ">1st</th>
                <th className="  py-2 text-[16px] font-semibold ">2nd</th>
                <th className="  py-2 text-[16px] font-semibold ">3rd</th>
                <th className="  py-2 text-[16px] font-semibold ">4th</th>
                <th className="  py-2 text-[16px] font-semibold ">5th</th>
                <th className="  py-2 text-[16px] font-semibold ">6th</th>
                <th className="  py-2 text-[16px] font-semibold ">7th</th>

                <th className="  py-2 text-[16px] font-semibold ">Action</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                listData?.map((item: any, index: any) => (
                  <tr
                    key={index}
                    className={` hover:bg-bg_card cursor-pointer text-left text-[14px] ${
                      index % 2 === 0 && "bg-bg_orange"
                    }`}
                  >
                    {/* Your code to render each row */}

                    <td className=" py-[8px]">{item?.session?.session_year}</td>
                    <td className=" py-[8px]">{item?.session_starting_date}</td>
                    <td>{item?.first_semester_end_date}</td>
                    <td>{item?.second_semester_end_date}</td>
                    <td>{item?.third_semester_end_date}</td>
                    <td>{item?.fourth_semester_end_date}</td>
                    <td>{item?.fifth_semester_end_date}</td>
                    <td>{item?.sixth_semester_end_date}</td>
                    <td>{item?.seventh_semester_end_date}</td>
                    ...
                    <td>
                      {
                        <div className=" flex items-center justify-center gap-[10px]">
                          ...
                          <div
                            ...
                            onClick={() => deleteSemester(item?.id)}
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="19"
