...
const Institute = (props: Props) => {
  ...
  const getAll = async () => {
    ...
    const res = await axios.get(
      EndPoint.GET_POLYTECHNIC + `?search=${searchType} ${searchInstitute}`,
      {
        params: {
          search: `${searchType} ${searchInstitute}`,
          limit: pageSize,
  ...
  const deleteInstitute = async (p_id: any) => {
    try {
      const res = await axios.delete(EndPoint.DELETE_POLYTECHNIC + p_id);
      getAll();
      toast.success("Polytechnic is deleted.");
    } catch (error: any) {
      if (error?.response) {
  ...
  useEffect(() => {
    getAll();
  }, [searchType, searchInstitute, pageNo, pageSize]);
  useEffect(() => {
    getAllDepartment();
  }, []);
  return (
        ...
        <div className=" grid grid-cols-3 gap-[16px]">
          ...
          <div className=" flex items-center gap-[16px]">
            <Select
              ...
              className=" w-[260px] text-[18px] h-[40px] text-black cursor-pointer"
            />
          </div>
          ...
          <div className=" flex items-center">
            <input
              ...
              className=" border-[1px] outline-none rounded-bl-[8px] rounded-tl-[8px] w-[605px] px-[24px] h-[40px] "
          ...
          <table className="w-full text-left  text-[#121212] ">
            <thead className=" bg-[#DBDDE2] ">
              <tr>
                ...
                <th className=" py-2 text-[16px] font-semibold ">Type</th>
                <th className=" py-2 text-[16px] font-semibold ">Department</th>
                <th className=" py-2 text-[16px] font-semibold ">Code</th>
            ...
            <tbody>
              {isLoading ? (
                <>
                  <TableRowSkeleton colomNumber={5} />
                  <TableRowSkeleton colomNumber={5} />
                  <TableRowSkeleton colomNumber={5} />
                  <TableRowSkeleton colomNumber={5} />
                  <TableRowSkeleton colomNumber={5} />
                ...
                listData?.map((item: any, index) => (
                  <tr
                    key={index}
                    className={` hover:bg-bg_card cursor-pointer text-left ${
                      index % 2 === 0 && "bg-bg_orange"
                    }`}
                  >
                    {/* Your code to render each row */}

                    <td className=" pl-1 py-[16px] font-semibold">
                      {item?.polytechnic_name}
                    </td>
                    <td className=" py-[16px]">{item?.institute_type}</td>
                    <td className=" py-[16px] text-left list-decimal gap-[24px]">
                      {item?.departments?.map((item: any, index: any) => (
                        <li key={index}>{item.full_name}</li>
                      ))}
                    </td>
                    <td className=" pl-1 py-[16px] font-semibold">
                      {item?.polytechnic_code}
                    </td>
