...
const TargetControl = (props: Props) => {
  const [toggle, setToggle] = useState(false);
  const [editToggle, setEditToggle] = useState(false);
  const [listData, setListData] = useState([]);
  const [catagoryId, setCatagoryId] = useState<any>(null);
  const [deleteWarn, setDeleteWarn] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  const getAll = async () => {
    setIsLoading(true);
    const res = await axios.get(EndPoint.GET_ALL_TARGET);
  ...
  const clikOndeleteButton = (item_id: any) => {
    setCatagoryId(item_id);
    setDeleteWarn(true);
  };

  const deleteCatagory = async () => {
    try {
      const res = await axios.delete(EndPoint.DELETE_TARGET + catagoryId);
      toast.success("Catagory is deleted.");
      setDeleteWarn(false);
      getAll();
    } catch (error) {
      toast.error("Catagory is not deleted.");
      setDeleteWarn(false);
      console.log(error);
    }
  };
  ...
  return (
    <div className=" flex flex-col gap-[24px]">
      ...
      <div className=" border-[1px] border-gray_light ">
        <table className="w-full table-fixed  text-left text-[#121212] ">
          <thead className=" bg-[#DBDDE2] sticky top-0 z-10">
            <tr>
              <th className=" py-2 text-[16px] font-semibold ">
                Category Name
              </th>
              <th className=" py-2 text-[16px] font-semibold ">Month</th>
              <th className=" py-2 text-[16px] font-semibold ">Year</th>
              <th className=" py-2 text-[16px] font-semibold ">Quantity</th>
