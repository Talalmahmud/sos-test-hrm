...
const Department = (props: Props) => {
  ...
  const [departmentId, setDpartmentId] = useState<any>(null);
  ...
  const getAll = async () => {
    setIsLoading(true);
    const res = await axios.get(EndPoint.GET_DEPARTMENT, {
      params: {
        limit: pageSize,
  ...
  const deleteCatagory = async (id: any) => {
    try {
      const res = await axios.delete(EndPoint.DELETE_DEPARTMENT + id);
      getAll();
          ...
          <tbody>
            {isLoading ? (
              ...
              listData?.map((item: any, index: any) => (
                ...
                >
                  <td>{`${item?.full_name} (${item?.short_name})`}</td>
