...
const Duet = (props: Props) => {
  ...
  const getAll = async () => {
    const res = await axios.get(EndPoint.GET_DUET_SCHEDUL);
    const resData = await res.data;
  ...
  const deleteExam = async (examId: any) => {
    try {
      const res = await axios.delete(EndPoint.DELETE_DUET_SCHEDUL + examId);
      getAll();
          ...
          <table className="w-full table-fixed text-left text-[#121212] ">
            <thead className=" bg-[#DBDDE2] ">
              <tr>
                ...
                <th className=" py-2 text-[16px] font-semibold ">Exam Date</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                listData?.map((item: any, index: any) => (
                  <tr
                    key={index}
                    className={` hover:bg-bg_card cursor-pointer ${
