...
const Session = (props: Props) => {
  ...
  const getAllProbidan = async () => {
    const res = await axios.get(EndPoint.GET_DROPDOWN_PROBIDAN);
    const resData = await res.data;
    setProbidanList(resData?.results);
  };
  const getAll = async () => {
    setIsLoading(true);
    const res = await axios.get(EndPoint.GET_SESSION);
    const resData = await res.data;
  ...
  const deleteSession = async (id: any) => {
    try {
      const res = await axios.delete(EndPoint.DELETE_SESSION + id);
      toast.success("Session delete is success");
    } catch (error: any) {
      if (error?.response) {
        ...
        <div className=" flex items-center gap-[24px]">
          ...
          {/* <div className=" flex items-center gap-[24px]">
            <Select
              ...
              className=" w-[227px] text-[20px] h-[40px] text-black cursor-pointer"
            />
          </div> */}
          ...
          <div className=" flex items-center">
            <input
              ...
              className=" border-[1px] outline-none rounded-bl-[8px] rounded-tl-[8px] w-[900px] px-[24px] h-[40px] "
          ...
          <table className="w-full  text-[#121212] ">
            <thead className=" bg-[#DBDDE2] ">
              <tr className=" text-left">
                ...
                <th className=" py-2 text-[16px] font-semibold ">Sesssion</th>
            ...
            <tbody>
              {isLoading ? (
                <>
                  <TableRowSkeleton colomNumber={4} />
                  <TableRowSkeleton colomNumber={4} />
                  <TableRowSkeleton colomNumber={4} />
                  <TableRowSkeleton colomNumber={4} />
                  <TableRowSkeleton colomNumber={4} />
                ...
                listData?.map((item: any, index: any) => (
                  <tr
                    key={index}
                    className={` hover:bg-bg_card cursor-pointer align-top text-left ${
                      index % 2 === 0 && "bg-bg_orange"
                    }`}
                  >
                    <td className=" ">{item?.probidhan_year}</td>
                    <td className="pb-[16px]">
                      {item?.probidhan_session?.map((item: any, index: any) => (
                        <p key={index}>{item?.session_year} ,</p>
                      ))}
                    </td>
                    <td className="  ">
                      {item.status ? (
                    ...
                    <td>
                      ...
                      {editToggle && (
                        <CustomModal setClose={setEditToggle}>
                          <div className=" rounded-[16px] bg-button_bg text-white py-[32px] w-[500px] flex flex-col gap-4 items-center justify-center ">
                            {listData?.map(
                              (outerItem: any, outerIndex: any) => (
                                <div key={outerIndex}>
                                  <p>{outerItem.probidhan_year}</p>
                                  {outerItem?.probidhan_session?.map(
                                    (innerItem: any, innerIndex: any) => (
                                      <SessionEdit
                                        key={innerIndex}
                                        session_year={innerItem.session_year}
                                        session_id={innerItem.id}
                                        listUpdate={getAll}
                            ...
                            <div className=" flex justify-end items-center gap-[16px]">
                              <button
                                ...
                                className=" flex justify-center items-center hover:bg-green  text-white w-[115px] h-[39px] border-[1px] border-white rounded-[8px]"
                              >
                                Cancel
                              </button>
                            </div>
