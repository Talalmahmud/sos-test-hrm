...
const UserInfo = ({ userDetails, userId }: Props) => {
  return (
    <div className=" w-full p-[16px] bg-orange rounded-[16px] flex gap-[24px] items-center ">
      <div className=" flex flex-col gap-[16px]">
        <div className=" w-[408px] h-[275px] rounded-[8px] bg-[#FFF7F4] p-[16px] flex flex-col items-center gap-[8px] justify-center">
          <div className=" w-[111px] h-[111px] bg-white rounded-full"></div>
          <div className=" flex items-center gap-[8px]">
            <p className=" text-[31px] font-bold">{userDetails?.name}</p>
            <Link href={`/management/user/update/${userId}`}>
              <svg
          ...
          <p className=" text-[16px]">
            {userDetails?.designation || "No position"}
          </p>
          <p className=" text-[16px]">Join:{userDetails?.join_date} </p>
        </div>

        <div className=" grid grid-cols-2 gap-[16px] items-center ">
          <div className=" h-[73px] bg-[#FFF7F4] rounded-[8px] flex flex-col items-center justify-center">
            <p className=" tex[13px]">All students</p>
            <p className=" text-[20px] font-bold">500</p>
          </div>
          <div className=" h-[73px] bg-[#FFF7F4] rounded-[8px] flex flex-col items-center justify-center">
            <p className=" tex[13px]">Enrolled Student</p>
            <p className=" text-[20px] font-bold">50</p>
          </div>
        </div>
      </div>
      <div className=" w-[408px]  rounded-[8px] bg-[#FFF7F4] flex flex-col gap-[8px] p-[16px] text-[16px]">
        <div className=" flex flex-col ">
          <p className=" font-bold">Company:</p>
          <li className=" ml-[8px] list-disc">Softmax online school</li>
        </div>
        <div className=" flex flex-col">
          <p className=" font-bold">Mobile Number:</p>
          <li className=" ml-[8px] list-disc">{userDetails?.phone_number}</li>
        </div>
        <div className=" flex flex-col">
          <p className=" font-bold">E-mail:</p>
          <li className=" ml-[8px] list-disc">{userDetails?.email}</li>
        </div>
        <div className=" flex flex-col">
          <p className=" font-bold">NID Number:</p>
          <li className=" ml-[8px] list-disc">{userDetails?.NID}</li>
        </div>
        <div className=" flex flex-col">
          <p className=" font-bold">Address:</p>
          <li className=" ml-[8px] list-disc">{userDetails?.address}</li>
        </div>
        <div className=" flex flex-col">
          <p className=" font-bold">Educational Qualification:</p>
          <li className=" ml-[8px] list-disc">{userDetails?.qualification}</li>
        </div>
      </div>

      <div className=" flex-1 h-[364px] rounded-[8px] bg-green p-[16px] flex flex-col items-center gap-[16px] justify-center">
        <p className=" text-[31px] leading-[24px] font-semibold text-white">
          Holiday
        </p>
        <div className=" w-full  flex flex-col items-center gap-[8px] justify-center rounded-[8px] bg-[#FFF7F4]">
          <p className=" text-[20px] font-semibold"> Official</p>
          <div className=" flex w-full justify-between items-center text-[13px] px-[16px]">
