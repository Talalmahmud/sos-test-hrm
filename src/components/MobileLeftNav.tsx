...
const MobileLeftNav = (props: Props) => {
  const router = usePathname();
  const { user, globalToggle, setGolbalToggle } = useContext(AuthContext);
  ...
  return (
    ...
    <div
      className={`  h-[600px] overflow-scroll no-scrollbar z-10 lg:hidden  bg-white border-r-[1px] px-[24px] py-[24px] border-gray_light rounded-[16px] shadow-md ${
        globalToggle
          ? " fixed lg:hidden md:top-[90px] md:left-0 z-40 "
          : "hidden"
      } flex flex-col items-center gap-[12px]`}
    >
      {linkList?.map((item, index) => (
        <Link
          onClick={() => setGolbalToggle(false)}
          href={item.urlLink}
          key={index}
          className={`${item?.role?.includes(user?.role) ? "flex" : "hidden"}`}
        >
          {/* ${
              item?.role?.includes(user?.role) ? "flex" : "hidden"
            } */}
          <div
            className={` ${
              router.includes(item.pathMatch) ? "bg-title border-white" : ""
            } flex ${
              globalToggle ? "pl-[8px]" : "px-[8px]"
            }  items-center gap-[4px] ${
              globalToggle ? "w-[300px]" : ""
